...
- **Внешние ссылки открываются системой, а не внутри WebView.** Страницы домена
  игры (включая `www.`) грузятся в приложении, всё остальное уходит через
  `Intent.ACTION_VIEW`: ссылку на Telegram подхватывает приложение Telegram,
  донат открывается в браузере. Без этого WebView пытался сам открыть `tg://…`
  и падал с «ошибка при загрузке tg:resolve?domain=…» — он понимает только
  `http`/`https`. Если Telegram не установлен, ссылка открывается как обычная
  веб-страница `t.me`.
- `usesCleartextTraffic="true"` — нужен, только если сервер работает по `http://`.
  При работе по HTTPS (как сейчас, `https://dvinsk.lat`) строку можно убрать
  из `AndroidManifest.xml`.
- Включён `domStorageEnabled` — игра хранит в localStorage токен сессии, данные
  аккаунта, имя, язык и настройку звука; без него не работали бы вход и
  переподключение.
