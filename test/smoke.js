...
ok('дамп валетов накрывает шестёрку и завершает раунд', () => {
  ...
  g.dumpJacks(g.players[who].token);           // дамп накрывает шестёрку и завершает раунд
...
ok('валет последней картой играется без масти', () => {
  const g = freshGame(2);
  force(g, { top: { r: '7', s: '♠' } });
  g.mustCoverSix = false;
  g.pendingSeven = g.pendingQueen = g.pendingSkip = false;
  const who = g.turn;
  const other = g.nextActiveIdx(who);
  g.players[who].hand = [{ r: 'J', s: '♥', id: 'J♥' }];      // единственная карта
  g.players[other].hand = [{ r: 'A', s: '♣', id: 'A♣' }, { r: 'A', s: '♦', id: 'A♦' }]; // 30
  g.playCard(g.players[who].token, 'J♥');                    // масть НЕ передаём
  assert.notStrictEqual(g.phase, 'playing', 'раунд завершён');
  assert.strictEqual(g.jackSuit, null, 'масть не заказана');
  assert.strictEqual(g.players[other].score, 60, '30 × 2 (финиш валетом)');
});

ok('валет не последней картой всё ещё требует масть', () => {
  const g = freshGame(2);
  force(g, { top: { r: '7', s: '♠' } });
  g.mustCoverSix = false;
  g.pendingSeven = g.pendingQueen = g.pendingSkip = false;
  const who = g.turn;
  g.players[who].hand = [{ r: 'J', s: '♥', id: 'J♥' }, { r: '10', s: '♣', id: '10♣' }];
  let threw = false;
  try { g.playCard(g.players[who].token, 'J♥'); } catch { threw = true; }
  assert(threw, 'без масти положить нельзя');
  g.playCard(g.players[who].token, 'J♥', '♣');
  assert.strictEqual(g.jackSuit, '♣', 'масть заказана');
});

ok('дамп валетов не требует масти', () => {
  const g = freshGame(2);
  force(g, { top: { r: '7', s: '♠' } });
  g.mustCoverSix = false;
  g.pendingSeven = g.pendingQueen = g.pendingSkip = false;
