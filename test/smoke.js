...
ok('на туза нельзя класть младшую той же масти (6 или король)', () => {
  const g = freshGame();
  force(g, { top: { r: 'A', s: '♠' } });
  assert(!g.canPlayCard({ r: '6', s: '♠' }));   // шестёрку — нельзя
  assert(!g.canPlayCard({ r: 'K', s: '♠' }));   // короля — нельзя
  assert(g.canPlayCard({ r: 'A', s: '♥' }));    // другого туза — можно
  assert(g.canPlayCard({ r: '9', s: '♦' }));    // девятку/валета — можно
});
...
ok('на 9 можно ответить девяткой (9 на 9)', () => {
  ...
  assert(!g.canPlayCard({ r: '6', s: '♣' }));           // та же масть младше — нельзя
});
